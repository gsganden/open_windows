...
class WeatherInputs:
    ...
    max_rh: int = 60
    latitude: float | None = (
        None  # Make optional or keep default? Let's make optional for now
    )
    longitude: float | None = None  # Make optional for now
    indoor_ref_temp: int = 69  # New user input field
    max_aqi: int = 100  # *** ADDED AQI Threshold ***
...
def find_optimal_periods(
    weather_data: dict, aqi_data: dict | None, inputs: WeatherInputs
) -> ForecastResult:
    """Identify periods and collect data, considering weather and AQI."""
    if (
        not weather_data
        or "hourly" not in weather_data
        or "time" not in weather_data["hourly"]
    ):
        return ForecastResult(
            periods=[],
            info="Error: Could not parse weather data.",
            daily_chart_data={},
            daily_good_intervals={},
            daily_good_periods_text={},
        )
    ...
    try:
        api_timezone_str = weather_data.get(
            "timezone", "UTC"
        )  # *** ADDED: Get timezone string for logging ***
        tz = pytz.timezone(api_timezone_str)
    except pytz.UnknownTimeZoneError:
        api_timezone_str = (
            "UTC (Fallback)"  # *** ADDED: Update string if fallback used ***
        )
        tz = pytz.utc
    ...
    if indoor_ref_temp_c is None:
        return ForecastResult(
            periods=[],
            info="Error: Invalid indoor reference temperature.",
            daily_chart_data={},
            daily_good_intervals={},
            daily_good_periods_text={},
        )
    ...
    for i, time_str in enumerate(times):
        ...
        dt_local = tz.localize(
            dt_utc
        )  # NEW: Localize the naive time using the API's timezone

        predicted_rh_in = None
        is_good = False
        temp_ok, rh_ok, precip_ok, precip_prob_ok, aqi_ok = (
            False,
            False,
            False,
            False,
        ...
        primary_data_ok = (
            temp_out_f is not None
            and dew_point_out_f is not None
        ...
        if primary_data_ok:
            ...
            if predicted_rh_in is not None:
                rh_ok = inputs.min_rh <= predicted_rh_in <= inputs.max_rh
            precip_ok = precip < 0.1
            precip_prob_ok = precip_probability <= inputs.max_precip_prob
            if aq_available:
                aqi_ok = aqi is not None and aqi <= inputs.max_aqi
            else:
                aqi_ok = True  # Assume OK if AQI not available
        ...
        if is_good and overall_start_time is None:
            overall_start_time = dt_local
            ...
            # --- End log REMOVAL ---
            start_fmt = overall_start_time.strftime("%a %I:%M %p")
            end_fmt = end_time.strftime("%a %I:%M %p")
            if overall_start_time.date() == end_time.date():
    ...
    if hourly_data_list:
        ...
        for (
            ...
            if day_str not in daily_chart_data_map:
                if daily_start_dt is not None:
                    end_dt = dt_local
                    daily_good_intervals_map[current_day_str].append(
                        (daily_start_dt, end_dt)
                    )
                    prev_day_last_hour = dt_local - timedelta(hours=1)
                    start_fmt = daily_start_dt.strftime("%I:%M %p")
                    end_fmt = (prev_day_last_hour + timedelta(hours=1)).strftime(
                        "%I:%M %p"
                    )
                    daily_good_periods_text_map[current_day_str].append(
                        f"{start_fmt} - {end_fmt}"
                    )
                    daily_start_dt = None
                ...
                daily_good_intervals_map[current_day_str] = []
                daily_good_periods_text_map[current_day_str] = []
            daily_chart_data_map[current_day_str].append(
                (dt_local, is_good_status, temp_f, rh_in, p, aqi_val, precip_prob)
            )
            if is_good_status and daily_start_dt is None:
                daily_start_dt = dt_local
        ...
        if daily_start_dt is not None and last_dt_local is not None:
            end_dt = last_dt_local + timedelta(hours=1)
            daily_good_intervals_map[current_day_str].append((daily_start_dt, end_dt))
            start_fmt = daily_start_dt.strftime("%I:%M %p")
            end_fmt = end_dt.strftime("%I:%M %p")
            daily_good_periods_text_map[current_day_str].append(
                f"{start_fmt} - {end_fmt}"
    ...
    shapes = [
        {
            ...
            "x1": e.isoformat(),
            "y1": 1,
            "fillcolor": "rgba(147, 217, 147, 0.3)",
            "line": {"width": 0},
        }
    ...
    shapes.append(
        {
            ...
            "yref": "y3",
            "x0": 0,
            "y0": inputs.max_precip_prob,
            "x1": 1,
            "y1": inputs.max_precip_prob,
...
def create_precip_chart_script(
    day_str: str,
    hourly_data: list[
        tuple[
            datetime,
            ...
            [
                {
                    ...
                    "yref": "y",
                    "x0": 0,
                    "y0": max_aqi_threshold,
                    "x1": 1,
                    "y1": max_aqi_threshold,
                    "line": {"color": "red", "width": 1, "dash": "dash"},
                }
            ]
        )
        if max_aqi_threshold is not None
        else "[]"
    )
...
async def get(inputs: WeatherInputs):
    ...
    # *** Fallback: Use provided lat/lon if location string wasn't used or failed, and lat/lon are valid ***
        ...
        inputs.latitude is not None
        and inputs.longitude is not None
        and not (-90 <= inputs.latitude <= 90 and -180 <= inputs.longitude <= 180)
    ):
    ...
    if address_str:
        ...
        lat is not None and lon is not None and not geocoding_error
    ):  # If reverse lookup failed but we have coords
        address_display = P(
            Strong("Approximate Location: "),
            f"Lat: {lat:.4f}, Lon: {lon:.4f} (Address lookup failed)",
            Cls="mb-3 text-muted",
        )
    elif not geocoding_error:  # If geocoding failed initially
        address_display = P(
            Strong("Approximate Location: "),
            "Unknown (Geocoding failed)",
            Cls="mb-3 text-muted",
        )
    ...
    if not geocoding_error:
        if inputs.min_temp >= inputs.max_temp:
            validation_error = "Min temp >= max temp."
        elif inputs.min_rh >= inputs.max_rh:
            validation_error = "Min RH >= max RH."
        elif inputs.max_aqi < 0:
            validation_error = "Max AQI cannot be negative."
    ...
    if geocoding_error:
        ...
        results_div = Div(
            P(f"Input Error: {validation_error}"), Cls="error-message", id="results"
        )
        ...
        if not result.daily_chart_data:
            ...
            sorted_days = sorted(
                result.daily_chart_data.keys(),
                key=lambda d: datetime.strptime(d, "%a %Y-%m-%d"),
            )
            ...
            for day_str in sorted_days:
                ...
                # Chart IDs and Divs
                main_chart_id = (
                    f"chart-div-{day_str.replace(' ', '-').replace('/', '-')}"
                )
                aqi_chart_id = (
                    f"aqi-chart-div-{day_str.replace(' ', '-').replace('/', '-')}"
                )
    ...
    form = Form(
        ...
        Fieldset(  # Location - NEW
            Label(
                "Location: ",
                Input(name="location", type="text", value=inputs.location or ""),
            )
        ),
        Fieldset(  # Temp
            Label(
                "Min Outdoor Temp (°F): ",
                Input(name="min_temp", type="number", value=inputs.min_temp),
            ),
            Label(
                "Max Outdoor Temp (°F): ",
                Input(name="max_temp", type="number", value=inputs.max_temp),
            ),
        ),
        Fieldset(  # RH
            Label(
                "Min Indoor Rel Hum (%): ",
                Input(
                    name="min_rh", type="number", value=inputs.min_rh, min=0, max=100
                ),
            ),
            Label(
                "Max Indoor Rel Hum (%): ",
                Input(
                    name="max_rh", type="number", value=inputs.max_rh, min=0, max=100
                ),
            ),
        ),
        Fieldset(  # Indoor Temp Ref
            Label(
                "Reference Indoor Temp (°F): ",
                Input(
                    name="indoor_ref_temp", type="number", value=inputs.indoor_ref_temp
                ),
            )
        ),
        Fieldset(  # Precip Probability
            Label(
                ...
                Input(
                    name="max_precip_prob",
                    type="number",
                    value=inputs.max_precip_prob,
                    min=0,
                    max=100,
                ),
            )
        ...
        Fieldset(  # AQI Threshold
            Label(
                "Max US AQI: ",
                Input(name="max_aqi", type="number", value=inputs.max_aqi, min=0),
            )
        ),
    ...
    if address_display:
        page_elements.append(address_display)
